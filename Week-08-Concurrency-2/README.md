...
This week is about blocked systems, thread state, lock order, and live debugging discipline.
...
```bash
cd Week-08-Concurrency-2/example
make clean
make
./deadlock
```
...
Live debugging is evidence preservation. A hung process is a crime scene: inspect thread stacks, waits, and lock order before disturbing it.

The mature fix is a lock-order rule or synchronization design, not a longer timeout.
