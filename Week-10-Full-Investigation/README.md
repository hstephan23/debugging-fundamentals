...
Choose one failure shape: crash, hang, invalid state, data loss, nondeterminism, failed integration, incorrect output, or slowness. Your job is to run a complete investigation and defend the postmortem.
...
Pick one previous example or a small failure from any stack you understand. Start with one playbook:

```bash
sed -n '1,220p' Week-10-Full-Investigation/playbooks.md
```

Then run the first evidence source the playbook recommends. Examples from the included systems fixtures:
...
- explain why each evidence source was chosen
