...
This week is about making nondeterminism observable and separating data races from higher-level race conditions.
...
```bash
cd Week-07-Concurrency-1/example
make clean
make
./race_counter
./race_counter
...
Concurrent bugs punish casual proof. A clean investigation names the shared state, identifies the racing accesses, and explains the synchronization rule that removes the race.
