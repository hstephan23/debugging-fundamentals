...
The same C source behaves differently in debug and release builds. In the debug build, the program is expected to trip an assertion. In the release build, the assertion is compiled out, so the symptom changes.

Your task is not to guess which build is "real." Your task is to treat the build mode as an input and explain what each run proves.
...
Do not use "debug build" or "release build" as an explanation by itself. Name the specific compiler flag, runtime check, optimization behavior, or hidden input you think matters.
...
```bash
cd Week-01-Foundations/example
make clean
make debug
make release
make run-debug
...
- Which warning flags would you keep in every C project?

## Debrief

This week is about controlled learning. Debugging starts by naming the symptom and changing one variable at a time. Build flags, environment, timing, and input are part of the bug surface.
...
- identify at least one useful warning flag
