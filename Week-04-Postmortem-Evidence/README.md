# Week 4 - Postmortem Evidence

## Investigation

A program failed outside your interactive debugging session. You do not get to rerun casually and hope the same thing happens. Your first job is to preserve postmortem evidence and inspect the state the system left behind.

This week is about debugging after the fact: crash artifacts, inputs, logs, traces, dumps, screenshots, request IDs, and any state that may disappear when you touch the system.
...
```bash
cd Week-04-Postmortem-Evidence/example
make clean
make
ulimit -c unlimited
...
Postmortem artifacts are evidence snapshots. They let you ask what the system knew at failure time, not what a later rerun happens to show. The mature debugging move is to preserve the artifact, verify it is readable, inspect the strongest evidence, and only then decide what to rerun.
...
- identify what production failure reports should capture
